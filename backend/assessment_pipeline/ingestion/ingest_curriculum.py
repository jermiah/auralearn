...
from assessment_pipeline.ingestion.ocr_client import MistralOCRClient
from assessment_pipeline.ingestion.parser import CurriculumChunker
from assessment_pipeline.ingestion.metadata import MetadataBuilder
from assessment_pipeline.ingestion.schemas import CurriculumChunk
...
def parse_ocr_to_chunks(ocr_result: Dict[str, Any]) -> List[CurriculumChunk]:
    """
    Parse OCR result into curriculum chunks

    Args:
        ocr_result: OCR result with pages data

    Returns:
        List of validated CurriculumChunk objects
    """
...
def main():
    ...
    # Configuration
    PDF_FOLDER = Path("pdfs")  # Local PDF folder

    # Validate PDF folder exists
    if not PDF_FOLDER.exists():
        print(f"\n[ERROR] PDF folder not found: {PDF_FOLDER.absolute()}")
        print(f"\n   Create folder and add curriculum PDFs:")
        print(f"   mkdir pdfs")
        print(f"   # Then add your PDF files to pdfs/")
        sys.exit(1)
    ...
    if not pdf_files:
        ...
        print(f"   Add PDF files to the pdfs/ folder")
        sys.exit(1)
    ...
    print(f"\n[SUCCESS] Curriculum data is now ready in Supabase!")
